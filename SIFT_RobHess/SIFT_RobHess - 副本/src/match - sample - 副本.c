...
int main( int argc, char** argv )
{
  ...
  cvResize(img1, down1, CV_INTER_CUBIC);//这种线性插值比最近邻得到的psnr大
    ...
    if( H1 )
      {
		...
		for (int i = 0; i < H1->rows; i++)//行  
		{
			for (int j = 0; j < H1->cols; j++)
			{
				...
				Ha[(i + 1)*(j + 1)-1] = (float)cvGetReal2D(H1, i, j);
